"""
Train loan collection risk prediction model
"""
...
from config import (
    COLLECTION_RISK_PROCESSED_FILE, COLLECTION_RISK_MODEL, RANDOM_STATE, TEST_SIZE
)
from utils.common import load_data
from utils.metrics import evaluate_classification_model
from utils.model_io import save_model
from preprocessing.collection_preprocessing import preprocess_collection_data


def train_collection_model():
    """
    Train and evaluate collection risk prediction models
    ...
    print("\n" + "="*50)
    print("Training Collection Risk Model")
    print("="*50)
    ...
    df = load_data(COLLECTION_RISK_PROCESSED_FILE)
    if df is None:
        print("✗ Preprocessed data not found. Run preprocessing first.")
        return None
    
    # Separate features and target
    X = df.drop('collection_risk', axis=1)
    y = df['collection_risk']
    ...
    # Save best model
    save_model(best_model, COLLECTION_RISK_MODEL)
